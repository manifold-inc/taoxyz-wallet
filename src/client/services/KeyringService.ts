...
export const KeyringService = {
  ...
  getWallet(address: string): KeyringPair | Error {
    ...
    if (!wallet) {
      ...
      console.log(
        '📋 [KeyringService] Available wallet addresses:',
        keyring.getPairs().map(w => w.address)
      );
  ...
  async sign(
    ...
    console.log('✍️ [KeyringService] Starting sign process for address:', address);

    const wallet = await this.getWallet(address);
    if (wallet instanceof Error) {
      console.error('❌ [KeyringService] Failed to get wallet:', wallet.message);
      return new Error(wallet.message);
    }

    console.log('✅ [KeyringService] Got wallet, current locked state:', wallet.isLocked);

    try {
      console.log('🔓 [KeyringService] Calling unlockWallet...');
      const unlockResult = this.unlockWallet(address, password);
      console.log('🔓 [KeyringService] unlockWallet returned:', unlockResult);

      console.log('🔍 [KeyringService] Checking wallet locked state after unlock...');
      console.log('🔍 [KeyringService] Wallet.isLocked:', wallet.isLocked);

      if (wallet.isLocked) {
        console.error('❌ [KeyringService] Wallet still locked after unlock attempt');
        return new Error('Wallet is Locked');
      }

      console.log('✅ [KeyringService] Wallet unlocked successfully, proceeding with signature...');
      ...
      console.log('✅ [KeyringService] Signature generated successfully');
      return signature;
    } catch (error) {
      console.error('💥 [KeyringService] Sign failed with error:', error);
  ...
  testUnlock(address: string, password: string): { success: boolean; details: string[] } {
    ...
    details.push(`   Contains unicode: ${password.split('').some(c => c.charCodeAt(0) > 127)}`);
